...
pub fn create_version(project_id: i32, name: String) -> Result<Version, String> {
    ...
    let tables: Vec<(String, String, String, Option<String>)> = table_stmt
        .query_map(params![project_id], |row| {
    ...
    for (table_id, table_name, display_name, comment) in &tables {
        ...
        let columns: Vec<ColumnDef> = col_stmt
            ...
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| format!("Error reading columns: {}", e))?;

        // 3. 获取索引
        let mut idx_stmt = conn
            .prepare(
                "SELECT id, table_id, name, index_type, comment FROM t_index WHERE table_id = ?1",
            )
            .map_err(|e| format!("Error preparing index stmt: {}", e))?;
        let mut indexes: Vec<IndexDef> = idx_stmt
            .query_map(params![table_id], |row| {
                Ok(IndexDef {
                    id: row.get(0)?,
                    table_id: row.get(1)?,
        ...
        for idx in &mut indexes {
            ...
            idx.fields = field_stmt
                .query_map(params![idx.id], |row| {
                    Ok(IndexField {
                        column_id: row.get(0)?,
                ...
                .collect::<Result<Vec<_>, _>>()
                .map_err(|e| format!("Error reading index fields: {}", e))?;
        }

        // 4. 获取元数据
        let mut data_stmt = conn
            .prepare("SELECT data FROM t_init_data WHERE table_id = ?1 ORDER BY id")
            .map_err(|e| format!("Error preparing init data stmt: {}", e))?;
        let init_data: Vec<String> = data_stmt
            .query_map(params![table_id], |row| row.get(0))
            .map_err(|e| format!("Error querying init data: {}", e))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| format!("Error reading init data: {}", e))?;
    ...
    let snapshot_routines: Vec<RoutineDef> = routine_stmt
        .query_map(params![project_id], |row| {
            Ok(RoutineDef {
                id: row.get(0)?,
                project_id: row.get(1)?,
        ...
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| format!("Error reading routines: {}", e))?;
...
pub fn export_upgrade_sql(
    ...
    let old_snap: Snapshot = serde_json::from_str(&old_json)
        .map_err(|e| format!("Error parsing old snapshot: {}", e))?;
    let new_snap: Snapshot = serde_json::from_str(&new_json)
        .map_err(|e| format!("Error parsing new snapshot: {}", e))?;
            ...
            for col in &new_table.columns {
                if let Some(old_col) = old_cols.get(&col.name) {
                    ...
                    let comment_text = col
                        ...
                        .filter(|c| !c.is_empty())
                        .unwrap_or(&col.display_name);
                    let old_comment_text = old_col
                        .comment
                        .as_deref()
                        .filter(|c| !c.is_empty())
                        .unwrap_or(&old_col.display_name);
                    ...
                    if type_changed || default_changed || ai_changed || comment_changed {
                        ...
                        if !col.nullable {
                            type_with_suffix.push_str(dialect.not_null_clause());
                        }
                        if col.auto_increment {
                            type_with_suffix.push_str(dialect.auto_increment_suffix());
                        }
                        if col.default_null {
                            type_with_suffix.push_str(" DEFAULT NULL");
                        } else if let Some(dv) = &col.default_value {
                            if !dv.is_empty() {
                                type_with_suffix.push_str(&dialect.default_value_clause(dv));
                            }
                        }
                        if dialect.supports_inline_comment() {
                            if !comment_text.is_empty() {
                                type_with_suffix.push_str(&format!(
                                    " COMMENT '{}'",
                                    comment_text.replace('\'', "''")
                                ));
                            }
                    ...
                    if comment_changed && !dialect.supports_inline_comment() {
                        let cs =
                            dialect.column_comment_sql(&new_table.name, &col.name, comment_text);
                        if !cs.is_empty() {
                            comment_changes.push(cs);
                        }
                    }
    ...
    if !old_routines.is_empty() || !new_routines.is_empty() {
        ...
        if !added_routines.is_empty()
            || !removed_routines.is_empty()
            || !modified_routines.is_empty()
        {
            sql.push_str("-- 编程对象变更\n\n");
...
pub fn export_project_sql(project_id: i32, database_type: String) -> Result<String, String> {
    ...
    let mut table_stmt = conn
        .prepare(
            "SELECT id, name, display_name FROM t_table WHERE project_id = ?1 ORDER BY created_at",
        )
        .map_err(|e| format!("Error: {}", e))?;
    let tables: Vec<(String, String, String)> = table_stmt
        .query_map(params![project_id], |row| {
    ...
    for (table_id, table_name, display_name) in &tables {
        ...
        for (name, disp_name, dt, len, scale, nullable, _pk, ai, dv, dn, cmt) in &cols {
            ...
            if *dn {
                ...
                if !d.is_empty() {
                    def.push_str(&dialect.default_value_clause(d));
                }
            }
            if dialect.supports_inline_comment() {
                let comment_text = cmt
                    .as_deref()
                    .filter(|c| !c.is_empty())
                    .unwrap_or(disp_name.as_str());
                if !comment_text.is_empty() {
                    def.push_str(&format!(" COMMENT '{}'", comment_text.replace('\'', "''")));
                }
            }
        ...
        // Indexes
        let mut idx_stmt = conn
            .prepare("SELECT id, name, index_type FROM t_index WHERE table_id = ?1")
            .map_err(|e| format!("Error: {}", e))?;
        ...
        for (idx_id, idx_name, idx_type) in &indexes {
            let mut field_stmt = conn
                .prepare(
                    "SELECT column_id FROM t_index_field WHERE index_id = ?1 ORDER BY sort_order",
                )
                .map_err(|e| format!("Error: {}", e))?;
            let col_ids: Vec<String> = field_stmt
                .query_map(params![idx_id], |row| row.get(0))
                .map_err(|e| format!("Error: {}", e))?
                .collect::<Result<Vec<_>, _>>()
                .map_err(|e| format!("Error: {}", e))?;
            let mut name_stmt = conn
                .prepare("SELECT name FROM t_column WHERE id = ?1")
                .map_err(|e| format!("Error: {}", e))?;
            let resolved_names: Vec<String> = col_ids
                .iter()
                .map(|cid| {
                    name_stmt
...
pub fn export_table_sql(table_id: String, database_type: String) -> Result<String, String> {
    ...
    let (table_name, display_name): (String, String) = conn
        .query_row(
            "SELECT name, display_name FROM t_table WHERE id = ?1",
            params![table_id],
    ...
    for (idx_id, idx_name, idx_type) in &indexes {
        let mut field_stmt = conn
            .prepare("SELECT column_id FROM t_index_field WHERE index_id = ?1 ORDER BY sort_order")
            .map_err(|e| format!("Error: {}", e))?;
        let col_ids: Vec<String> = field_stmt
            .query_map(params![idx_id], |row| row.get(0))
            .map_err(|e| format!("Error: {}", e))?
