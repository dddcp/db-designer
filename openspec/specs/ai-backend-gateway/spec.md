...
后端 SHALL 提供 `ai_chat` Tauri 命令，接受会话消息列表与本地 AI 配置，调用供应商的 chat completion 接口并返回助手回复文本；该命令 MUST 在内部统一完成 baseUrl 到 chat URL 的推导，不依赖前端预先拼接路径。请求体 SHALL 保持模型默认行为（思考开启），不再附加控制思考链的字段；该规则同样适用于流式聊天命令。
